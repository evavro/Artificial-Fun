...
//-------------------------- ctor ---------------------------------------------
//-----------------------------------------------------------------------------
Slug::Slug(AbstRaven_Bot* shooter, Vector2D target):
...
void Slug::TestForImpact()
{
  ...
  std::list<AbstRaven_Bot*> hits = GetListOfIntersectingBots(m_vOrigin, m_vPosition);

  //if no bots hit just return;
  if (hits.empty()) return;

  //give some damage to the hit bots
  std::list<AbstRaven_Bot*>::iterator it;
