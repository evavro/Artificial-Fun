...
public:
  ...
  virtual double CalculateDesirability(AbstRaven_Bot* pBot)=0;
  
  //adds the appropriate goal to the given bot's brain
  virtual void  SetGoal(AbstRaven_Bot* pBot) = 0;

  //used to provide debugging/tweaking support
  virtual void  RenderInfo(Vector2D Position, AbstRaven_Bot* pBot) = 0;
};
