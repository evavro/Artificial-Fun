...
#include "Raven_BotSteeringBehaviors.h"
...
//-------------------------- ctor ---------------------------------------------
Raven_Bot::Raven_Bot(Raven_Game* world,Vector2D pos): AbstRaven_Bot(world, pos)       
{
	m_pScript = Raven_BotScriptor::Instance();
  ...
  m_pSteering = new Raven_BotSteering(world, this);
...
std::string const Raven_Bot::GetName () const { return "Buckland"; }

//-------------------------------- Update -------------------------------------
//
void Raven_Bot::DoUpdate()
{
...
void			Raven_Bot::SelectBodyPen()
{
	gdi->BluePen();
}
void			Raven_Bot::SelectHeadPen()
 {
	 gdi->BrownPen();
 }
