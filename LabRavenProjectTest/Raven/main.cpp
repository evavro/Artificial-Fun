...
char*	g_szWindowClassName = "RavenWindowClass";
    ...
    case WM_KEYUP:
      {
        switch(wParam)
        {
         ...
         case VK_UP:
           ...
           g_pRaven->AddBot("Raven_Bot"); break;
...
void ReportScores ()
{
	const double TIME_DIVISOR = 1.0e06;
	std::string infoString = "Game Over\n";
	std::list<AbstRaven_Bot*> bots = g_pRaven->GetAllBots();
	// determine winning time
	std::list<AbstRaven_Bot*>::iterator it = bots.begin();
	int winningScore = -1;
	double winningAdjScore = -1.;
	double winningTime = 0.;
	double adjScore;
	double botTime;
	std::string winningBotIdString;
	for (it; it != bots.end(); ++it)
	{
		AbstRaven_Bot* pbot = (*it);
		// if two bots tie for the most kills, the one with more time is used as the
		// benchmark, and the one with less time gets a boost.
		int score = pbot->Score() - pbot->Deaths();
		if ((score >= winningScore))
		{
			if ((score != winningScore) || (pbot->GetFloatTime() >= winningTime))
			{	
				winningScore = score;
				winningTime = pbot->GetFloatTime();
			}
		}
	}
	it = bots.begin();
	...
	{
		AbstRaven_Bot* pbot = (*it);
		std::string botIdString = "Bot: " + pbot->GetName() + "-" + ttos(pbot->ID()) + "\n";
		std::string botScoreString = "\tKills = " + ttos(pbot->Score());
		std::string botDeathString = "\tDeaths = " + ttos(pbot->Deaths()) + "\n";
		botTime = pbot->GetFloatTime();
		std::string botTimeString  = "\tTime = " + ttos(botTime) + "\n";
		adjScore = 1.0 * (pbot->Score() - pbot->Deaths());
		if (botTime < winningTime)
		{
			adjScore += (5. - 5. * botTime / winningTime);
		}
		if (adjScore > winningAdjScore)
		{
				   winningBotIdString = "\n\nWinner is Bot: " 
					+ pbot->GetName()+ "-" + ttos(pbot->ID()) + "\n";
				   winningAdjScore = adjScore;
		}
		else if(adjScore==winningAdjScore)
					winningBotIdString += "and Bot: " 
					+ pbot->GetName()+ "-" + ttos(pbot->ID()) + "\n(It's a tie!)\n";  
		std::string botAdjScore = "\tAdj. Score = " + ttos(adjScore) + "\n";

		infoString = infoString + botIdString + botScoreString + botDeathString + botTimeString + botAdjScore;
	}
    infoString += winningBotIdString;
	debug_on
  debug_con << infoString;
  debug_con << "";
  
  debug_off
	MessageBox(NULL, infoString.c_str(), "Final Result", 0);
}
