...
private:
  ...
  //if unsuccessful 
  bool AttemptToAddBot(AbstRaven_Bot* pBot);
  ...
  //their memory
  void NotifyAllBotsOfRemoval(AbstRaven_Bot* pRemovedBot)const;
...
public:
  ...
  void AddBot(std::string botClassName);
  void AddRocket(AbstRaven_Bot* shooter, Vector2D target);
  void AddRailGunSlug(AbstRaven_Bot* shooter, Vector2D target);
  void AddShotGunPellet(AbstRaven_Bot* shooter, Vector2D target);
  void AddBolt(AbstRaven_Bot* shooter, Vector2D target);
  ...
  std::vector<AbstRaven_Bot*> GetAllBotsInFOV(const AbstRaven_Bot* pBot)const;

  //returns true if the second bot is unobstructed by walls and in the field
  //of view of the first.
  bool        isSecondVisibleToFirst(const AbstRaven_Bot* pFirst,
  ...
  //method returns NULL
  AbstRaven_Bot*  GetBotAtPosition(Vector2D CursorPos)const;
  ...
  AbstRaven_Bot*  PossessedBot()const{return m_pSelectedBot;}
  ...
  const std::list<AbstRaven_Bot*>&             GetAllBots()const{return m_Bots;}
