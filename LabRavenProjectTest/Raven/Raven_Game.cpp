                         ...
                         m_pGraveMarkers(NULL)
{
  ...
  m_gameDuration = script->GetInt("GameDuration");
}
...
void Raven_Game::Clear()
{
  ...
  //delete the bots
  std::list<AbstRaven_Bot*>::iterator it = m_Bots.begin();
...
{ 
  ...
  std::list<AbstRaven_Bot*>::iterator curBot = m_Bots.begin();
  ...
  { 
    if (!m_Bots.empty())
    {
      AbstRaven_Bot* pBot = m_Bots.back();
...
//----------------------------- AttemptToAddBot -------------------------------
//-----------------------------------------------------------------------------
bool Raven_Game::AttemptToAddBot(AbstRaven_Bot* pBot)
{
...
//  their memory
//-----------------------------------------------------------------------------
void Raven_Game::NotifyAllBotsOfRemoval(AbstRaven_Bot* pRemovedBot)const
{
    std::list<AbstRaven_Bot*>::const_iterator curBot = m_Bots.begin();
    for (curBot; curBot != m_Bots.end(); ++curBot)
    {
...
//--------------------------- AddBolt -----------------------------------------
//-----------------------------------------------------------------------------
void Raven_Game::AddBolt(AbstRaven_Bot* shooter, Vector2D target)
{
...
//------------------------- AddRailGunSlug -----------------------------------
void Raven_Game::AddRailGunSlug(AbstRaven_Bot* shooter, Vector2D target)
{
...
//------------------------- AddShotGunPellet -----------------------------------
void Raven_Game::AddShotGunPellet(AbstRaven_Bot* shooter, Vector2D target)
{
...
//-----------------------------------------------------------------------------
AbstRaven_Bot* Raven_Game::GetBotAtPosition(Vector2D CursorPos)const
{
  std::list<AbstRaven_Bot*>::const_iterator curBot = m_Bots.begin();
...
{
  AbstRaven_Bot* pBot = GetBotAtPosition(POINTStoVector(p));
...
Raven_Game::GetAllBotsInFOV(const AbstRaven_Bot* pBot)const
{
  std::vector<AbstRaven_Bot*> VisibleBots;
...
bool
Raven_Game::isSecondVisibleToFirst(const AbstRaven_Bot* pFirst,
                                   const AbstRaven_Bot* pSecond)const
{
...
bool Raven_Game::isTimeUp ()
{
	return (gameTimeMin() >= m_gameDuration);
}

time_t Raven_Game::gameTimeMin ()
{
	time_t currentTime = time(NULL);
	time_t diff = currentTime - m_startTime;
	float minutesSoFar = diff / 60.0f;
	return (time_t) minutesSoFar;
}

time_t Raven_Game::gameTimeSec ()
{
	time_t currentTime = time(NULL);
	time_t diff = currentTime - m_startTime;
	float minutesSoFar = diff / 60.0f;
	return diff - ((int) minutesSoFar * 60);
}
